...
class FinetuningArguments(
    ...
    # ============ Joint SDFT (Self-Distillation Fine-Tuning) ============
    use_joint_sdft: bool = field(
        default=False,
        metadata={"help": "Whether to use Joint Self-Distillation Fine-Tuning."},
    )
    sdft_mode: str = field(
        default="joint",
        metadata={
            "help": (
                "SDFT mode: "
                "'joint' = single model dual forward pass (teacher+student share params); "
                "'ema' = dual model, teacher updated via EMA from student."
            )
        },
    )
    sdft_kl_weight: float = field(
        default=0.1,
        metadata={"help": "KL distillation loss weight in student total loss."},
    )
    sdft_distill_temperature: float = field(
        default=1.0,
        metadata={"help": "Distillation temperature. Higher (2-5) transfers more dark knowledge."},
    )
    sdft_teacher_ce_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for teacher CE loss (only used in 'joint' mode)."},
    )
    sdft_ema_decay: float = field(
        default=0.999,
        metadata={"help": "EMA decay coefficient (only used in 'ema' mode)."},
    )
    sdft_keyword_pattern: str = field(
        default=r"，相关的商品关键词有.*?(?=，请给出)",
        metadata={"help": "Regex to remove privileged keywords from instruction for student."},
    )
